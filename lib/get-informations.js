...
function get_power(emeter_number,cb) {
  req(function (err, save) {
    if (err) return cb(err);
    cb(null, save.emeters[emeter_number].power);
  });
}
...
module.exports = { req: req, get_moving_save: get_moving_save, get_moving_average_power: get_moving_average_power, update: update, get_power: get_power };
